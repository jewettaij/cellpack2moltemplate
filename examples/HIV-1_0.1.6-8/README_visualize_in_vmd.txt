Note: VMD may be a poor choice when looking at the kinds of complex systems
created by CellPACK.  (Ovito might work a lot better, but it's render
quality does not always look as pretty as VMD.)  These instructions are for VMD.

 ------- To view a lammps trajectory in VMD --------


1) Build a PSF file for use viewing trajectories with VMD.

If you ran moltemplate.sh using the "-vmd" flag, then a
"system.psf" file should have already been created for you.
(If so, then you can skip this step.)
...
2) Later, to Load a trajectory in VMD:
...
d) Try entering these commands:
    ...
    pbc wrap -compound res -all -shiftcenterrel {-0.5 -0.5 0.0}
    pbc box -shiftcenterrel {-0.5 -0.5 0.0} -width 0.5
